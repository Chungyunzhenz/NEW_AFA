"""Migration: Add feature_importance_json column to model_registry table.

SQLite does not support full ALTER TABLE, but ADD COLUMN works.

Usage:
    python scripts/migrate_add_feature_importance.py
    python scripts/migrate_add_feature_importance.py --check
"""
...
def migrate(check_only: bool = False) -> None:
    ...
    column = "feature_importance_json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add feature_importance_json column to model_registry table.",
    )
    parser.add_argument(
        "--check",
        action="store_true",
