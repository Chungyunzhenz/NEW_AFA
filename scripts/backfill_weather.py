"""Backfill historical weather observations from the CWA API.

Usage:
    python scripts/backfill_weather.py [--start YYYY-MM-DD] [--end YYYY-MM-DD]

By default the script fetches weather data for the last 3 years up to
yesterday.  Handles keyboard interrupts gracefully.
"""
...
from app.services.weather_collector import CWAWeatherCollector  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill CWA weather observations for a date range.",
    )
    parser.add_argument(
        "--start",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="CWA API key (overrides CWA_API_KEY env variable).",
    )
    parser.add_argument(
        "--rate-limit",
        type=float,
...
def main() -> None:
    ...
    logger.info(
        "Backfilling weather data from %s to %s (%d days).",
        start_date,
        end_date,
        total_days,
    )
    ...
    collector = CWAWeatherCollector(api_key=args.api_key)
    ...
    try:
        while current <= end_date:
            ...
            try:
                ...
                if inserted > 0:
                    logger.info("  -> inserted %d county records.", inserted)
            except Exception:
    ...
    logger.info(
        "Backfill finished: %d days processed, %d total weather records inserted.",
        days_processed,
        total_inserted,
    )
